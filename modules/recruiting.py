    ...
    if job_skill_ids:
        ...
        for skill in skills:
            ...
            if skill_id and skill_type_id:
                ...
                skill_level_id = random.choice(level_entries)["id"] if level_entries else None
                line = {"skill_id": skill_id, "skill_type_id": skill_type_id}
                if skill_level_id:
                    line["skill_level_id"] = skill_level_id
...
def _create_skills(client, recruiting_data: dict, num_skill_types: int, skills_per_type: int):
    ...
    for skill_type_data in recruiting_data.get("skill_types", [])[:num_skill_types]:
        ...
        if skill_type_name.lower() in existing_skill_types:
            print(f"-> Kompetenzart '{skill_type_name}' existiert bereits, überspringe")
...
def _create_jobs(client, gemini, ctx, recruiting_data, num_jobs, departments, all_skill_ids, skill_levels_map):
    if not departments:
        print("⚠️  Keine Abteilungen — Jobs übersprungen.")
        return []
    ...
    print(f"✅ {len(job_ids)} Stellen erstellt.")
    return job_ids
    ...
    for job_idx, job_id in enumerate(job_ids):
        ...
        for _ in range(count):
            ...
            create_applicant(
                ...
                job_id,
                names[candidate_idx],
                emails[candidate_idx],
                phones[candidate_idx],
